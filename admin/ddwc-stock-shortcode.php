<?php
/**
 * The stock manage dashboard Shortcode.
 *
 ...
 */
function stock_manage_dashboard_shortcode() {
...
?>
<!--  stock filters  -->
<form class="order-filters" method='post' action="<?php $_SERVER['REQUEST_URI'] ?>">
	<div class="form-group">
		<label>Date:</label><input type="date" name="filter-date" value="<?php $_POST['filter-date'] ?>">
	</div>
	<div class="form-group">
		<label>Driver</label>
	<select name="filter-name">
		<option selected="selected" value="<?php $_POST['filter-name'];?>"></option>
		<?php				
		$user_query = new WP_User_Query(array( 'role' => 'driver' ));
if ( ! empty( $user_query->get_results() ) ) {
	foreach ( $user_query->get_results() as $user ) {
		echo '<option value="'. $user->ID .'">'.$user->display_name.' User ID: ('.$user->ID.')'.' </option>';
	}
...
<?php
					// 	stock variables
						...
						if($_SERVER['REQUEST_METHOD'] == 'POST'){
							$driver_id = $_POST['filter-name'];
							if(isset($_POST['filter-date'])){
								$time = strtotime($_POST['filter-date']);
							$year = date('Y', $time);
							$month = date('n', $time);
							$day = date('j', $time);
...
add_shortcode('stock-dashboard' , 'stock_manage_dashboard_shortcode');
// [print_button]
function print_button_shortcode( $atts ){
return '<button class="print-link" onclick="javascript:window.print()">Print</button>';
}
